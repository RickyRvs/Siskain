<x-app-layout>
    ...
    <div class="py-6"
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-28">
            ...
            <form id="posForm" action="{{ route('transactions.store') }}" method="POST" @submit="beforeSubmit">
                @csrf
                ...
                {{-- Hidden inputs keranjang, ikut ke-submit walau tampilannya di dalam modal --}}
                ...
                <!-- ==================== KATALOG PRODUK (full-width, selalu) ==================== -->
                <div class="space-y-4">
                    <!-- Search + kategori (sticky) -->
                    <div class="sticky top-0 z-10 -mx-4 sm:mx-0 px-4 sm:px-0 pt-1 pb-2 space-y-3 bg-[#FAF8F2]/95 backdrop-blur-sm">
                        <div class="relative">
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-3 sm:gap-4">
                        <template x-for="p in filteredProducts()" :key="p.id">
                            <button type="button" @click="addToCart(p)" :disabled="p.tracks_stock && p.stock <= 0 && !p.has_variant"
                                    class="group relative text-left bg-white rounded-2xl ring-1 ring-[#E7E1D3] overflow-hidden hover:ring-[#D4A73C] hover:shadow-lg hover:-translate-y-0.5 active:scale-[0.97] transition-all duration-150 disabled:opacity-50 disabled:hover:translate-y-0 disabled:hover:shadow-none disabled:cursor-not-allowed">

                                <!-- Badge qty di keranjang -->
                                <span x-show="qtyInCart(p.id) > 0" x-cloak
                                      class="absolute top-2 right-2 z-10 min-w-[20px] h-5 px-1.5 rounded-full bg-[#D4A73C] text-[#1F2A24] text-[11px] font-bold flex items-center justify-center shadow-sm"
                ...
                <div x-show="cartOpen" x-cloak
                    ...
                    <div class="relative bg-white w-full h-full sm:h-auto sm:w-full sm:max-w-md sm:max-h-[88vh] sm:rounded-2xl sm:shadow-2xl flex flex-col"
                        ...
                        <div class="px-5 py-4 border-b border-[#F0ECE0] shrink-0 flex items-center justify-between">
                            ...
                            <button type="button" @click="cartOpen = false" class="text-[#8A8272] hover:text-[#1F2A24] p-1">
                        ...
                        <div class="flex-1 overflow-y-auto">
                            ...
                            <div class="px-5 mt-3 divide-y divide-[#F0ECE0]">
                                        ...
                                        <div class="flex-1 min-w-0">
                                            ...
                                            <div class="flex items-center gap-1.5 mt-1.5">
                                                <button type="button" @click="changeQty(index, -1)"
                                                        class="w-7 h-7 flex items-center justify-center rounded-full border border-[#DDD5C2] text-[#5B5647] hover:bg-[#F6F3EC] active:scale-90 transition">
                                ...
                                <p x-show="items.length === 0" class="py-8 text-center text-sm text-[#B0A98F]">Keranjang masih kosong.</p>
                            </div>
                        </div>
                        ...
                        <div class="px-5 pt-4 pb-[calc(1rem+env(safe-area-inset-bottom))] border-t border-[#F0ECE0] bg-[#FAF8F2] space-y-3 shrink-0">
                            ...
                            <div>
                                ...
                                <select x-model="paymentMethod" class="w-full text-sm border-[#DDD5C2] rounded-md shadow-sm focus:border-[#D4A73C] focus:ring-[#D4A73C]" required>
                            ...
                            <button type="submit" :disabled="items.length === 0 || submitting"
                                ...
                                <span x-show="!submitting" x-text="'Simpan Transaksi — Rp ' + formatRp(total)"></span>
            ...
            <button type="button"
                    x-show="items.length > 0 && !cartOpen"
                    x-cloak x-transition
                    @click="cartOpen = true"
                    class="fixed bottom-5 right-4 sm:right-6 z-40 flex items-center gap-2.5 bg-[#1F2A24] text-white pl-4 pr-5 py-3.5 rounded-full shadow-[0_6px_20px_rgba(15,46,43,0.35)] hover:bg-[#16201B] active:scale-95 transition">
                <span class="relative">
                    <svg class="w-5 h-5 text-[#D4A73C]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <span class="absolute -top-2 -right-2 min-w-[18px] h-[18px] px-1 rounded-full bg-[#D4A73C] text-[#1F2A24] text-[10px] font-bold flex items-center justify-center"
                          x-text="items.length"></span>
                </span>
                <span class="text-sm font-semibold" x-text="'Rp ' + formatRp(total)"></span>
            </button>
        ...
        function posForm({ products }) {
            ...
            return {
                ...
                removeItem(index) {
                    ...
                    this.recalc();
                    if (this.items.length === 0) this.cartOpen = false;
                },

                setExactAmount() {
                    this.paidAmount = this.total;
                    this.paidAmountDisplay = this.formatRp(this.total);
                },
