    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
                ...
                <div class="bg-white rounded-xl ring-1 ring-[#E7E1D3] shadow-sm p-5 flex items-center gap-4">
                    <span class="w-10 h-10 rounded-lg bg-[#FBEAE6] text-[#B5482E] flex items-center justify-center shrink-0">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v3.75m9-.75a9 9 0 11-18 0 9 9 0 0118 0zm-9 3.75h.008v.008H12v-.008z" />
                        </svg>
                    </span>
                    <div class="min-w-0">
                        <p class="text-xs text-[#8A8272]">Langganan Kadaluarsa</p>
                        <p class="text-xl font-semibold text-[#B5482E]">{{ $summary['kadaluarsa'] }}</p>
                    </div>
                </div>
                <div class="col-span-2 sm:col-span-3 lg:col-span-1 bg-white rounded-xl ring-1 ring-[#E7E1D3] shadow-sm p-5 flex items-center gap-4">
                    ...
                    <div class="min-w-0">
                        ...
                        <p class="text-xl font-semibold text-[#1F2A24]" title="Rp {{ number_format($summary['omzet'], 0, ',', '.') }}">{{ $fmtRupiah($summary['omzet']) }}</p>
                    </div>
                </div>
            </div>
            ...
            <form method="GET" class="flex flex-col sm:flex-row sm:items-center gap-3">
                <div class="relative flex-1 sm:flex-none">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama usaha..."
                           class="w-full sm:w-64 rounded-lg border-[#DDD5C2] text-sm pl-9 focus:ring-[#D4A73C] focus:border-[#D4A73C]">
                </div>
                <div class="flex items-center gap-3 flex-wrap">
                    <select name="status" onchange="this.form.submit()"
                            class="flex-1 sm:flex-none rounded-lg border-[#DDD5C2] text-sm focus:ring-[#D4A73C] focus:border-[#D4A73C]">
                        <option value="">Semua Status</option>
                        <option value="aktif" {{ request('status') === 'aktif' ? 'selected' : '' }}>Aktif</option>
                        <option value="nonaktif" {{ request('status') === 'nonaktif' ? 'selected' : '' }}>Nonaktif</option>
                        <option value="kadaluarsa" {{ request('status') === 'kadaluarsa' ? 'selected' : '' }}>Langganan Kadaluarsa</option>
                    </select>
                ...
                @forelse ($tenants as $t)
                    ...
                    <div class="bg-white rounded-xl ring-1 ring-[#E7E1D3] shadow-sm overflow-hidden hover:shadow-md transition flex flex-col">
                        ...
                        <div class="p-5 flex-1">
                            <div class="flex items-start justify-between gap-3">
                                <div class="flex items-center gap-3 min-w-0">
                                    ...
                                    <div class="min-w-0">
                                        <a href="{{ route('superadmin.tenants.show', $t) }}" class="font-semibold text-[#1F2A24] truncate hover:text-[#B5842A] block">{{ $t->name }}</a>
                                        <p class="text-xs text-[#8A8272] truncate">{{ $t->slug }}</p>
                                    </div>
                                </div>
                                <span class="shrink-0 flex flex-col items-end gap-1">
                                    <span class="text-[10px] font-medium uppercase px-2 py-1 rounded-full {{ $t->is_active ? 'bg-[#EAF3EE] text-[#2F6F4E]' : 'bg-[#F2F2F2] text-[#8A8272]' }}">
                            ...
                            <div class="mt-4 pt-4 border-t border-[#EFEAE0] flex items-center gap-2 text-sm min-w-0">
                                ...
                                @if ($owner)
                                    ...
                                    <span class="text-[#8A8272] truncate hidden sm:inline">&middot; {{ $owner->email }}</span>
                                @else
                            ...
                            <div class="mt-4 grid grid-cols-2 gap-3">
                                ...
                                <div class="bg-[#F6F3EC] rounded-lg px-3 py-2 min-w-0">
                                    <p class="text-[10px] uppercase tracking-wide text-[#8A8272]">Total Transaksi</p>
                                    <p class="text-sm font-semibold text-[#1F2A24]" title="Rp {{ number_format($t->transactions_sum_total ?? 0, 0, ',', '.') }}">{{ $fmtRupiah($t->transactions_sum_total ?? 0) }}</p>
                                </div>
                            </div>
                        ...
                        <div class="flex items-center justify-end gap-1 px-4 py-3 bg-[#FAF8F2] border-t border-[#EFEAE0]">
                            <a href="{{ route('superadmin.tenants.show', $t) }}" title="Detail" aria-label="Detail {{ $t->name }}"
                               class="p-2 rounded-lg text-[#5B5647] hover:bg-[#F0ECE0] transition">
                                <svg class="w-[18px] h-[18px]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-3m-6-9h7m0 0v7m0-7L10 15" />
                                </svg>
                            </a>
                            <form method="POST" action="{{ route('superadmin.tenants.impersonate', $t) }}">
                                @csrf
                                <button type="submit" title="Masuk sebagai Tenant" aria-label="Masuk sebagai tenant {{ $t->name }}"
