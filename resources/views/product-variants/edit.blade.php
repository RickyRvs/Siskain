<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Edit Varian - {{ $product->name }}</h2>
    </x-slot>
            ...
            <div class="bg-white shadow rounded-lg p-6">
                <form action="{{ route('variants.update', $variant) }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="sku" value="{{ old('sku', $variant->sku) }}" class="w-full border-gray-300 rounded-md shadow-sm">
                    ...
                    <div class="grid grid-cols-2 gap-4 mb-4">
                        <div>
                            ...
                            <input type="number" step="0.01" name="price_modal" value="{{ old('price_modal', $variant->price_modal) }}" class="w-full border-gray-300 rounded-md shadow-sm" required>
                            @error('price_modal') <p class="text-red-600 text-sm mt-1">{{ $message }}</p> @enderror
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Harga Jual</label>
                            <input type="number" step="0.01" name="price_jual" value="{{ old('price_jual', $variant->price_jual) }}" class="w-full border-gray-300 rounded-md shadow-sm" required>
                            @error('price_jual') <p class="text-red-600 text-sm mt-1">{{ $message }}</p> @enderror
                        </div>
                    </div>

                    <p class="text-xs text-gray-500 mb-4">Stok saat ini: {{ $variant->stock }} (ubah lewat menu Stok, bukan di sini)</p>
